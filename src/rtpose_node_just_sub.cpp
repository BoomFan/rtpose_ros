...
// DEFINE_string(caffemodel, "/home/roahm/pose_ws/src/rtpose_ros/Lib/model/coco/pose_iter_440000.caffemodel", "Caffe model.");
// DEFINE_string(caffeproto, "/home/roahm/pose_ws/src/rtpose_ros/Lib/model/coco/pose_deploy_linevec.prototxt", "Caffe deploy prototxt.");
...
void getFrameFromROSandPublish(const sensor_msgs::ImageConstPtr& msg) {
    cv::Mat img;
    cv_bridge::CvImagePtr cv_ptr = cv_bridge::toCvCopy(msg, "rgb8");
    printf("Img copyed\n");
    //cv::imwrite("rgb.png", cv_ptr->image);
...
void publishImg(cv::Mat img){
    ros::NodeHandle nh_pub;
    printf("ROS Handle nh_pub started\n");
    image_transport::ImageTransport it_pub(nh_pub);
    image_transport::Publisher gPublisher = it_pub.advertise(PUBLISH_RET_TOPIC_NAME, 1);
    sensor_msgs::ImagePtr output = cv_bridge::CvImage(std_msgs::Header(), "rgb8", img).toImageMsg();
    //std_msgs::ImagePtr output = cv_bridge::CvImage(std_msgs::Header(), "rgb8", img).toImageMsg();
    ros::Rate loop_rate(5);
       while (nh_pub.ok()) {
         gPublisher.publish(output);
         ros::spinOnce();
         loop_rate.sleep();
       }
...
void* displayROSFrame(void *i) { //single thread
    Frame frame;
    int counter = 1;
    double last_time = get_wall_time();
    double this_time;
    ...
    while(1) {
        ...
        if (global.output_queue_ordered.try_pop(&frame)) {
            ...
            if (!FLAGS_no_display) {
                // cv::imshow("video", wrap_frame);
                // publishImg(wrap_frame);
                final_img = wrap_frame;
                ros::NodeHandle nh_pub;
                // printf("ROS Handle nh_pub started\n");
                image_transport::ImageTransport it_pub(nh_pub);
                image_transport::Publisher gPublisher = it_pub.advertise(PUBLISH_RET_TOPIC_NAME, 1);
...
int main(int argc, char *argv[]) {
    ...
    // image_transport::Publisher gPublisher = it.advertise(PUBLISH_RET_TOPIC_NAME, 1);
    ...
    // sensor_msgs::ImagePtr output = cv_bridge::CvImage(std_msgs::Header(), "rgb8", final_img).toImageMsg();
